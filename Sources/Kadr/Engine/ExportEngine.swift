...
internal enum ExportEngine {
    ...
    /// The writer path is **off**.
    ///
    /// `AssetWriterExporter` is written and its routing is tested, but its reader
    /// configuration is rejected — `startReading()` fails with `-11800 / -12710` on
    /// every composition. Routing real exports into it would break exports that
    /// currently succeed, so the gate stays shut until the pipeline works.
    ///
    /// Flipping this to `true` is the whole of enabling it; everything else is in
    /// place. See `AssetWriterExporter` for what has been ruled out so far.
    static let writerPathEnabled = false

    static func shouldUseWriter(bitrate: Int?, overlays: [any Overlay]) -> Bool {
        writerPathEnabled && bitrate != nil && overlays.isEmpty
    }
